...
const { stringSchema } = makePskHelpers([32]); // 256-bit PSK for security keys
...
// Rebuild SecuritySchema specifically for ConfigBackup import to handle flexible adminKey array
const ConfigBackupSecuritySchema = z.object({
  ...
  // Allow adminKey to be a variable length array of strings
...
// Custom ChannelSchema for backup that is more permissive with PSK lengths but validates structure
// We use Zod's native enum for Role to ensure it matches Protobuf values (which are numbers or strings in Zod native enum depending on definition, but here we likely get numbers or string keys)
// Protobuf.Channel.Channel_Role is an object mapping strings to numbers. z.nativeEnum handles this.
const RoleEnum = z.nativeEnum(Protobuf.Channel.Channel_Role);
...
const ParsedConfigBackupFieldSchema: z.ZodType<ParsedConfigBackupField> = z.object({
...
export class ConfigBackupValidationService {
  static validateConfigBackupStructure(data: unknown): ConfigBackupData {
    try {
      ...
      return ConfigBackupExportSchema.parse(data) as unknown as ConfigBackupData;
    } catch (error) {
