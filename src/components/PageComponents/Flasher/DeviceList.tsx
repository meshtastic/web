...
export const DeviceList = ({
  ...
  devices
    .filter((d) => d.flashState.state == "doFlash")
    .forEach((d) => configMap.set(d, configQueue.shift()));

  return (
    <div className="flex w-full  min-w-[250px] max-w-[400px] rounded-md border border-dashed border-slate-200 p-3 dark:border-slate-700">
      {devices.length ? (
        <div className="flex w-full flex-col justify-between overflow-y-auto overflow-x-clip">
              ...
              return (
                <DeviceSetupEntry
                  ...
                  toggleSelectedToFlash={() => {
                    if (overallFlashingState.state == "busy") return;
                    const newState: FlashState =
                      state.state == "doFlash"
                        ? { progress: 0, state: "doNotFlash" }
                        : { progress: 1, state: "doFlash" };
...
const DeviceSetupEntry = ({
  ...
  device: Device;
  configName?: string;
  toggleSelectedToFlash: () => void;
  progressText: FlashState;
}) => {
  const selectedToFlash = progressText.state == "doFlash";
  const buttonCaption = selectedToFlash
    ? configName ?? "Unassigned"
    : deviceStateToText(progressText);
  const buttonStyle = deviceStateToStyle(
    progressText,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Button
              ...
              size="sm"
              style={buttonStyle}
              className="inline-block h-8 w-[9rem] gap-2 overflow-hidden text-ellipsis whitespace-nowrap text-sm"
  ...
  switch (state.state) {
    ...
    default:
      return {
        ...
        background: `linear-gradient(90deg, var(--accentMuted) ${
          state.progress * 100
        }%, transparent ${state.progress * 100}%)`
      };
  }
}
