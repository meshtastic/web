...
type OverallFlashingCallback = (
  flashState: OverallFlashingState,
  progress?: number
) => void;
...
export async function setup(
  ...
  forceFullFlash: boolean,
  overallCallback: OverallFlashingCallback
) {
  dataSections = {};
...
export async function nextBatch(
  devices: Device[],
  flashStates: FlashState[],
  deviceCallback: (flashState: FlashOperation) => void
) {
  callback("busy");
  ...
  operations = devices.map(
    (dev, index) =>
      new FlashOperation(
        dev,
        configQueue[index].config,
        configQueue[index].moduleConfig,
        deviceCallback
      )
  );
  ...
  Promise.allSettled(operations.map((op) => op.flashAndConfigDevice())).then(
    (p) => handleFlashingDone()
  );
}
...
function handleFlashingDone() {
  if (configQueue.length == 0) callback("idle");
...
async function getZipFile() {
  if (firmwareToUse.inLocalDb) {
    ...
    if (storedZip !== undefined) return storedZip;
  }
  ...
  while (true) {
    ...
    callback(
      "downloading",
      totalLength ? bytesLoaded / totalLength : undefined
    );
  }
...
function autoDetectDeviceModel(port: SerialPort) {
  ...
  return deviceModels.find(
    (d) => info.usbVendorId == d.vendorId && info.usbProductId == d.productId
  )?.name;
}
...
export class FlashOperation {
  ...
  public constructor(
    ...
    public config: Protobuf.LocalConfig,
    public moduleConfig: Protobuf.LocalModuleConfig,
    public callback: (operation: FlashOperation) => void
  ) {}
  ...
  public setState(
    ...
    console.log(
      `${
        this.device.nodes.get(this.device.hardware.myNodeNum)?.user?.longName ??
        "<Not flashed yet>"
      } flash state: ${state}`
    );
  ...
  private async flash() {
    ...
    const update =
      !fullFlash &&
      this.device.nodes.get(this.device.hardware.myNodeNum) !== undefined;
    if (update && installedVersion == firmwareToUse.tag) return;

    const port = await (
      this.device.connection! as ISerialConnection
    ).disconnect();
    if (port === undefined) throw "Port unavailable";
    const deviceModel =
      selectedDeviceModel == "auto"
        ? autoDetectDeviceModel(port)
        : selectedDeviceModel;
    if (deviceModel === undefined) throw "Could not detect device model";
    const info = port.getInfo();
    console.log(
      `Device info: vendor ${info.usbVendorId}, product ${info.usbProductId}`
    );
    ...
    try {
      ...
      const totalLength = sections.reduce<number>(
        (p, c) => p + c.data.byteLength,
        0
      );
      let bytesFlashed = 0;
      let lastIndex = 0;
      ...
      await loader.write_flash(
        ...
        true,
        (index, written, total) => {
          if (this.isCancelled) throw "Cancelled";
          if (index != lastIndex) {
          ...
          const bytesThisSegment =
            (written / total) * sections[index].data.byteLength;
          console.log(
            `FLASHING PROGRESS ${
              bytesFlashed + written
            } / ${totalLength} ... ${bytesFlashed} | ${written} | ${total} | ${index}`
          );
          this.setState(
            "flashing",
            (bytesFlashed + bytesThisSegment) / totalLength
          );
        }
      );
    } catch (e) {
  ...
  private async setConfig() {
    ...
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "device", value: this.config.device! }
      })
    );
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "position", value: this.config.position! }
      })
    );
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "power", value: this.config.power! }
      })
    );
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "network", value: this.config.network! }
      })
    );
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "display", value: this.config.display! }
      })
    );
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "lora", value: this.config.lora! }
      })
    );
    await connection.setConfig(
      new Protobuf.Config({
        payloadVariant: { case: "bluetooth", value: this.config.bluetooth! }
      })
    );

    await connection.setModuleConfig(
      new Protobuf.ModuleConfig({
        payloadVariant: { case: "mqtt", value: this.moduleConfig.mqtt! }
      })
    );
    await connection.setModuleConfig(
      new Protobuf.ModuleConfig({
        payloadVariant: { case: "serial", value: this.moduleConfig.serial! }
      })
    );
    await connection.setModuleConfig(
      new Protobuf.ModuleConfig({
        payloadVariant: {
          ...
          value: this.moduleConfig.externalNotification!
        }
      })
    ),
      await connection.setModuleConfig(
        new Protobuf.ModuleConfig({
          payloadVariant: {
            case: "storeForward",
            value: this.moduleConfig.storeForward!
          }
        })
      ),
      await connection.setModuleConfig(
        new Protobuf.ModuleConfig({
          payloadVariant: {
            case: "telemetry",
            value: this.moduleConfig.telemetry!
          }
        })
      ),
      await connection.setModuleConfig(
        new Protobuf.ModuleConfig({
          payloadVariant: {
            case: "cannedMessage",
            value: this.moduleConfig.cannedMessage!
          }
        })
      ),
      await connection.setModuleConfig(
        new Protobuf.ModuleConfig({
          payloadVariant: { case: "audio", value: this.moduleConfig.audio! }
        })
      );

    // We won't get an answer if serial output has been disabled in the new config
    if (!this.config.device!.serialEnabled) return;
    await connection
      .commitEditSettings()
      .then(() => console.log("FLASHER: Config saved"));
  }
